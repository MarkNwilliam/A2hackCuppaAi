...
function VideoAdPage() {
  ...
  const handleSubmit = async (event) => {
    event.preventDefault();
      ...
      try {
        const combinedPrompt = `Make a Video using this information ${formData.businessName}. ${formData.description}. Industry: ${formData.industry}.`;

        const requestData = {
          prompt: combinedPrompt  // Use the combined prompt here
        };

        console.log(requestData)
    
        const response = await fetch(`https://api.mosiai.studio/app1//createReactiveVideo`, {
          method: 'POST',
          headers: {
        ...
        console.log(data)
        setVideoAdUrl(data.link);  // Assuming the video link is returned in a field named 'video_link'
